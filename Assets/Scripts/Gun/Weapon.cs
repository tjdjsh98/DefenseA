...
public class Weapon : MonoBehaviour, ITypeDefine
{
    ...
    public virtual void Reload(IWeaponUsable user)
    {
        ...
        if (Managers.GetManager<GameManager>().Inventory.GetIsHaveItem(ItemName.퀵드로우_A))
        {
            if (Random.Range(0, 100) < 10)
            {
                CompleteReload();
    ...
    public virtual void CompleteReload(float ammoMultifly = 1,bool isHideGauge = true)
    {
        if (_isAllReloadAmmo)
        {
            ...
            if (Managers.GetManager<GameManager>().Inventory.GetIsHaveItem(ItemName.과적재탄창))
            {
            ...
            else if (Managers.GetManager<GameManager>().Inventory.GetIsHaveItem(ItemName.과적재탄창_A))
            {
            ...
            else if (Managers.GetManager<GameManager>().Inventory.GetIsHaveItem(ItemName.과적재탄창_B))
            {
                if (Random.Range(0, 100) < 40)
                {
                    ammoMultifly += 1;
