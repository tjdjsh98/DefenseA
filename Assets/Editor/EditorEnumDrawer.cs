    ...
    : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var currentIndex = Mathf.Clamp(property.intValue, 0, valuesStr.Count);
        var currentName = valuesStr[currentIndex];
        ...
        if (GUI.Button(fieldRect, currentName, EditorStyles.popup))
        {
            ...
            {
                if ( fieldInfo.FieldType is ItemName)
                {
                    ItemName itemName = ItemName.None;
                    Enum.TryParse(str, true, out itemName);
                    property.serializedObject.Update();
                    property.intValue = (int)itemName;
